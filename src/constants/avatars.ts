...
const avatarDict: {[key: string]: InternalAvatar} = {
  ...
  supporter: {
    ...
    title: 'supporter',
    description:
      'verb (used with object), a·chieved, a·chiev·ing.  to bring to a successful end; carry through; accomplish: The police crackdown on speeders achieved its purpose.  to get or attain by effort; gain; obtain: to achieve victory. verb (used without object), a·chieved, a·chiev·ing.  to bring about an intended result; accomplish some purpose or effect.',
    imgs: [
...
export const avatars = flatten(
  Object.keys(avatarDict).map(k =>
    avatarDict[k].imgs.map(d => ({
      ...avatarDict[k],
...
export const avatarUrls :{[a:string]:string}= {
  user1,
  user2,
  user3,
  user4,
