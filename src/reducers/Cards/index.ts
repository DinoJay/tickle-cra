...
import allSoftSkills, {SoftSkill} from '~/constants/softSkillData';

import Topic from '~/constants/topicType';

import {TITLE, TEMP_ID, Card, LOC} from '~/constants/cardFields';
...
const defaultCardTemplate: Card & {template: boolean} = {
  // TODO: not perfect
  id: 'exampleId',
  uid: 'exampleUid',
...
// const cardTemplateId = 'temp';
function reducer(
  state: CardsStateType = INITIAL_STATE,
  ...
  switch (action.type) {
    ...
    case REMOVE_ACTIVITY_SUB: {
      ...
      const ca: Card | undefined = cards.find(c => c.id === id);
      if (!ca) return state;
      const nc = {
        ...ca,
      ...
      const newCards = cards.reduce(
        (acc: Card[], c: Card) => [...acc, c.id !== id ? c : nc],
        []
      );
      return {
        ...state,
    ...
    case CREATE_CARD: {
      ...
      const newCard: Card = action.options;
    ...
    case UPDATE_CARD: {
      const {cards} = state;
      const updatedCard: Card = action.options as Card;

      const doUpdateCard = (c: Card): Card => {
    ...
    case SUBMIT_ACTIVITY: {
      ...
      const activitySubmission: ActivitySubmission = action.options;

      const updatedCards = cards.map(c => {
        const allActivitySubs = c.allActivitySubs
          ? c.allActivitySubs.reduce(
              (acc: ActivitySubmission[], a: ActivitySubmission) => [
                ...acc,
    ...
    case PUT_REWARD: {
      const newReward: Reward = action.options;
      const {rewards}: {rewards: Reward[]} = state;

      const alreadyExist =
        rewards.find(d => d.id === newReward.id) || null;
    ...
    case DELETE_REWARD: {
      ...
      return {
        ...state,
        rewards: rewards.filter((d: Reward) => d.id !== id)
      };
    }
