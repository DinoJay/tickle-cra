...
//
import { doCreateEnv, doAddUserToEnv, doReadAllEnvs, doDeleteEnv
// doReadEnvs
 } from '~/firebase/db/env_db';
import { readAllUsers, readAllTmpUsers, doCreateTmpUser, doAddEnvToUser, doCreateUser, deleteUser, deleteTmpUser, doInviteUser, doRemoveEnvFromUser } from '~/firebase/db/user_db';
import { doReadAllEvents, doReadEventsFromUser } from '~/firebase/db/event_db';
import { receiveUsers, receiveAllEnvs, receiveAllEvents, 
// submitActivityReview,
// submitActivityReviewSuccess,
// selectUsersByEnv,
insertUserIntoEnv, deleteUserFromEnv, deleteEnv, addEnv, updateEnv as updateEnvAction, addUser, updateUserInfo, 
// userRegistrationError,
removeUserSuccess, removeUser } from './actions';
...
export function fetchAllEnvs() {
    return function (dispatch) {
        return doReadAllEnvs().then((envs) => {
...
export function removeEnv(envId) {
    return function (dispatch) {
...
//   };
// }
export function removeUserAccount(uid) {
    return function (dispatch) {
...
export function addUserToEnv({ envId, usrInfo }) {
    return function (dispatch) {
...
export function excludeUserFromEnv({ envId, usrInfo }) {
    return function (dispatch) {
        dispatch(updateUserInfo({
            ...usrInfo,
...
export function fetchAllUserEvents(email) {
    return function (dispatch) {
        doReadEventsFromUser(email).then((events) => {
