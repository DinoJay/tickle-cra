...
function BubbleSet() {
  ...
  function Line(_x1, _y1, _x2, _y2) {
    ...
    this.ptClose = function(x, y, r) {
      ...
      } else if (x < x2 - r || x > x1 + r) return false;
      if (y1 < y2) {
        if (y < y1 - r || y > y2 + r) return false;
      } else if (y < y2 - r || y > y1 + r) return false;
      return true;
    };
  } // Line
  ...
  Intersection.testIntersection = function(
    line,
    bounds,
    intersections
  ) {
  ...
  function MarchingSquares(contour, potentialArea, step, t) {
    ...
    this.march = function() {
      for (let x = 0; x < potentialArea.width() && !marched; x += 1) {
        for (
          let y = 0;
          y < potentialArea.height() && !marched;
          y += 1
        ) {
          if (
            potentialArea.get(x, y) > threshold &&
            getState(x, y) != 15
          ) {
            marched = doMarch(x, y);
          }
  ...
  this.createOutline = function(members, nonmem, edges) {
    ...
    const estLength =
      (Math.floor(activeRegion.width()) +
        Math.floor(activeRegion.height())) *
      2;
    ...
    for (let i = 0, j = 0; j < size; j += 1, i += thisSkip) {
      fhull.add(
        new Point(
          surface.get(i).x() + xcorner,
          surface.get(i).y() + ycorner
        )
      );
    }
        ...
        while (!hasIntersection && linesToCheck.length) {
          ...
          if (closestItem) {
            ...
            if (numIntersections === 2) {
              ...
              let pointInside = isPointInsideNonMember(
                movePoint,
                nonMembers
              );
  ...
  function countInterferenceItems(interferenceItems, testLine) {
    return interferenceItems.reduce(function(count, interferenceItem) {
      if (interferenceItem.intersectsLine(testLine)) {
        if (
          Intersection.fractionToLineCenter(
            interferenceItem,
            testLine
          ) >= 0
        ) {
    ...
    if (wrapNormal) {
      ...
      if (leftIntersect.getState() === Intersection.POINT) {
        ...
        if (topArea < totalArea * 0.5) {
          ...
          if (
            leftIntersect.getPoint().y() > rightIntersect.getPoint().y()
          )
            // top left
            return new Point(
              rectangle.minX() - rerouteBuffer,
      ...
      if (leftArea < totalArea * 0.5) {
        // go around left
        if (
          topIntersect.getPoint().x() > bottomIntersect.getPoint().x()
        )
          // top left
          return new Point(
            rectangle.minX() - rerouteBuffer,
...
export {ShapeSimplifier, BSplineShapeGenerator, PointPath};
